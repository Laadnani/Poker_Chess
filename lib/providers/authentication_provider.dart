...
// Sign in user with Google and update the state
Future<UserCredential?> signInWithGoogle(BuildContext context) async {
  try {
    ...
    // Once signed in, update the provider state
    final authProvider = context.read<AuthenticationProvider>();
    authProvider._uid = userCredential.user?.uid;
    authProvider._isSignedIn = true;

    // Save signed-in status in shared preferences
    await authProvider.setSignedIn();

    // Save user data to Firestore if necessary
     authProvider.saveUserDataToFireStore(
      currentUser: UserModel(
        uid: authProvider._uid!,
      ...
      fileImage: null, // if no image to upload
      onSuccess: () => print("User saved successfully"),
      onFail: (error) => print("Failed to save user: $error"),
    );
